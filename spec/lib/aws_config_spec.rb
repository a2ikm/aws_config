...
describe AWSConfig do
  ...
  before do
    allow(AWSConfig).to receive(:config_file).and_return(sample_config_file)
    allow(AWSConfig).to receive(:credentials_file).and_return(sample_creds_file)
  end

  it "returns an entry in a profile via method" do
  ...
  it "returns an entry in a profile like hashes" do
    expect(described_class["default"]["aws_access_key_id"]).to eq "DefaultAccessKey01"
    expect(described_class["default"]["region"]).to eq "us-west-1"
    expect(described_class["default"]["output"]).to eq "json"
